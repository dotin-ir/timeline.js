(function(window){
	...
	function createTimeline(){
		var TimeLine = {};
		TimeLine.create = function(selector, config){
            addTitle(selector, config.title);
            addTimeline(selector, config);
		};

		var persianDigits = ['۰', '۱', '۲',  '۳',  '۴',  '۵',  '۶',  '۷',  '۸', '۹'];
        var dowNames = ['شنبه', 'یک‌شنبه', 'دوشنبه',  'سه‌شنبه',  'چهارشنبه',  'پنج‌شنبه',  'جمعه'];

        function addTitle(selector, title) {
            if (title != 'disabled') {
                var titleElement = document.createElement('h1');
                titleElement.textContent = title;
                document.querySelector(selector).appendChild(titleElement);
            }
        }

        function addTimeline(selector, config) {
        ...
        function appendMonthForInterval(intervalRow, month, interval, swimlaneColor) {
            for (var day = 1; day <= month.numOfDays; day++) {
	...
	if(typeof(TimeLine) === 'undefined'){
		window.TimeLine = createTimeline();
	}
	else{
		console.log("Timeline already defined.");
	}
})(window);
